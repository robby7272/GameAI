...
public abstract class GoapPlanner implements IGoapPlanner {
	...
	public Queue<GoapAction> plan(IGoapUnit goapUnit) {
		...
		try {
			...
			if (this.goapUnit.getGoalState().get(0).importance == Integer.MAX_VALUE) {
				...
				goalState.add(this.goapUnit.getGoalState().get(0)); //Gets first goal state
				//Finds plan by creating a graph using goalState
				...
				//Finds plan by creating a graph without using goalState
	...
	protected IWeightedGraph<GraphNode, WeightedEdge> createGraph(List<GoapState> goalState) {
		...
		//Graph generated by DefaultGoapPlanner
		//Either graph generation is being messed up, or addVerticies doesn't work
		//Graph generation looks fine, it's extremely simple
		//Although I think it was one of the things we had to change in order to get this library working

		generatedGraph = addVertices(generatedGraph, goalState);
		addEdges(generatedGraph);

		//printInfo("Goal added to graph: " + goalState.get(0).effect);

		HashSet<WeightedEdge> edges = generatedGraph.getEdges();
		//printInfo("Edges: " + edges);
		//printInfo("Vertices: " + generatedGraph.getVertices());
	 ...
	 */
	protected IWeightedGraph<GraphNode, WeightedEdge> addVertices(IWeightedGraph<GraphNode, WeightedEdge> graph, List<GoapState> goalState) {
		...
		this.startNode.overwriteOwnProperties(start); //Overwrites startNode (null action) with null as precondition, and getWorldState()'s effects as the effects
		graph.addVertex(this.startNode);

		for (GoapState state : goalState) { //Will add our single goal GoapState ("can_reach_player")
			//printInfo("Adding goapState " + state + " to vertices");
			...
			for(GoapState i : end.preconditions)
			{
				//printInfo("Graph node:" + i.effect); //Returned a null pointer?
			}
			//New graphnode with goalstate as a precondition? (and the only precondition)
			graph.addVertex(end);

			this.endNodes.add(end);
		}
		HashSet<GoapAction> possibleActions = extractPossibleActions();
		//printInfo("# of possible actions: " + possibleActions.size()); //Returned 0. That's a problem...
		...
		//printInfo("# of graph vertices: " + graph.getVertices().size());
		return graph;
	}
	...
	protected HashSet<GoapAction> extractPossibleActions() {
		...
		try {
			for (GoapAction goapAction : this.goapUnit.getAvailableActions()) {
				//printInfo("Checking action");
				if (goapAction.checkProceduralPrecondition(this.goapUnit)) { //This condition fails
					//printInfo("Adding action");
	...
	protected Queue<GoapAction> searchGraphForActionQueue(IWeightedGraph<GraphNode, WeightedEdge> graph) {
		...
		//Loops through an arraylist of graphNodes
	 ...
	 */
	protected static Queue<GoapAction> extractActionsFromGraphPath(WeightedPath<GraphNode, WeightedEdge> path, GraphNode startNode, GraphNode endNode) {
